...
qbRT::Transform::Transform(const qbMatrix2<double> &fwd, const qbMatrix2<double> &bck)
{
	if (	(fwd.GetNumRows() != 4) || (fwd.GetNumCols() != 4) ||
		(bck.GetNumRows() != 4) || (bck.GetNumCols() != 4))
	{
		throw std::invalid_argument("Cannot construct Transform, inputs are not all 4x4.");
	}
...
void qbRT::Transform::SetTransform(const qbVector<double> &translation,
...
qbMatrix2<double> qbRT::Transform::GetForward()
{
	return m_fwdtfm;
}
qbMatrix2<double> qbRT::Transform::GetBackward()
{
	return m_bcktfm;
}

qbRT::Ray qbRT::Transform::Apply(const qbRT::Ray &inputRay, bool dirFlag)
{
	qbRT::Ray outputRay;
...
qbVector<double> qbRT::Transform::Apply(const qbVector<double> &inputVector, bool dirFlag)
{
...
namespace qbRT
{
	qbRT::Transform operator* (const qbRT::Transform &lhs, const qbRT::Transform &rhs)
	{
		...
		qbRT::Transform finalResult (fwdResult, bckResult);
		
		return finalResult;
	}
}

qbRT::Transform qbRT::Transform::operator= (const qbRT::Transform &rhs)
{
	if (this != &rhs)
	{
...
void qbRT::Transform::PrintMatrix(bool dirFlag)
{
	if (dirFlag)
	{
		Print(m_fwdtfm);
...
void qbRT::Transform::Print(const qbMatrix2<double> &matrix)
{
...
void qbRT::Transform::PrintVector(const qbVector<double> &inputVector)
{
