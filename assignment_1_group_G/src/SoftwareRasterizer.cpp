...
void SoftwareRasterizer::init()
{
    ...
    if (SDL_Init(SDL_INIT_EVERYTHING) < 0)
    {
        ...
        if (win == NULL)
        {
            ...
            // Rasterize the tick mark with superSampling

            for(auto i : indices){
                rasterTriangle(vertices[i[0]], vertices[i[1]], vertices[i[2]], colors[i[0]], colors[i[1]], colors[i[2]]);
            }
...
void SoftwareRasterizer::rasterTriangle(glm::vec4 vertex1, glm::vec4 vertex2,glm::vec4 vertex3, glm::vec4 colour1, glm::vec4 colour2, glm::vec4 colour3) 
{
    ...
    for(int i = start_x; i<=end_x ;i++){
        for(int j = start_y; j<=end_y; j++){
            ...
            if(isInside(Bottom[0],Bottom[1],Middle[0],Middle[1],Top[0],Top[1],i,j)){
                glm::vec4 point = glm::vec4(i,j,0,1);
                glm::vec4 color = interpolateColorOnTriangle(Bottom,Middle,Top,BottomColour,MiddleColour,TopColour,point);
